...
#Tkinter setup (GUI)
root = Tk()
screen = Canvas(root, width=500, height=600, background="grey",highlightthickness=0)
screen.pack()
...
class Board:
	def __init__(self):
		...
		#Initializing center values and counters
		self.array[3][3]="w"
		self.array[3][4]="b"
		self.array[4][3]="b"
		self.array[4][4]="w"
	...
	def update(self):
		...
		for x in range(8):
			for y in range(8):
				#Creating the shapes for the counters
			...
			for y in range(8):
				if self.player == 0:
					if valid(self.array,self.player,x,y):
						screen.create_oval(68+50*x,68+50*y,32+50*(x+1),32+50*(y+1),tags="highlight",fill="yellow",outline="yellow")
		...
		if not self.won:
			...
			screen.create_text(250,550,anchor="c",font=("Arial",15), text="The game is over!", fill = "red")
	...
	def drawScoreBoard(self):
		...
		screen.create_text(30,550,anchor="w", tags="score",font=("Arial", 50),fill="white",text=player_score)
		screen.create_text(400,550,anchor="w", tags="score",font=("Arial", 50),fill="black",text=computer_score)
	...
	#Medium AI - Chooses a move based on what will get it the most pieces next turn
	def MediumAI(self):
	...
	#Decent AI - Choose a move based on a simple heuristic
	#Same as MediumAI() just uses MediumScore()
	def decentMove(self):
		#Generates all possible choices and boards corresponding to those
		boards = []
		choices = []
		...
		bestScore = -float("inf")
		bestIndex = 0
		#Determines the best score based on the prior generated boards and a "Meh" Heuristic: MediumScore()
		for i in range(len(boards)):
			score= MediumScore(boards[i],self.player)
			if score>bestScore:
				bestIndex=i
				bestScore = score
		#Move to the best location based on MediumScore()
		self.arrayMove(choices[bestIndex][0],choices[bestIndex][1])
...
#Returns an updated board after making a move according to Reversi rules
...
def drawGridBackground(outline=True):
	...
	for i in range(8):
		...
		#Horizontal line
		screen.create_line(50,lineShift,450,lineShift,fill="black")

		#Vertical line
		screen.create_line(lineShift,50,lineShift,450,fill="black")
...
def finalHeuristic(array,player):
	if moves<=8:
		...
		return MediumScore(array,player)
	else:
...
def runGame():
	...
	running = False
	#Title and shadow
	screen.create_text(250,203,anchor="c",text="Reversi",font=("Arial", 50),fill="#aaa")
	screen.create_text(250,200,anchor="c",text="Reversi",font=("Arial", 50),fill="#fff")
	...
	for i in range(3):
		...
		for x in range(i+1):
			...
			screen.create_text(25+(x+1)*spacing+155*i,326,anchor="c",text="\u2605", font=("Arial", 25),fill="#b29600")
			screen.create_text(25+(x+1)*spacing+155*i,327,anchor="c",text="\u2605", font=("Arial",25),fill="#b29600")
			screen.create_text(25+(x+1)*spacing+155*i,325,anchor="c",text="\u2605", font=("Arial", 25),fill="#ffd700")
